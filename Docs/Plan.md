...
- Verified: Created `src/canvas/shapes/ArrayShapeUtil.ts`, `ArrayComponent.tsx`, and `arrayShapeLogic.ts`. Registered in `WhiteboardCanvas.tsx` via `shapeUtils={[ArrayShapeUtil]}` with `maxFontsToLoadBeforeRender: 0` for instant canvas readiness. Added continuous GPU sliding pointer track with spring easing. Implemented array shape resize persistence: `WhiteboardCanvas.tsx` preserves user-resized `w` and `h` without resetting them during step transitions; `ArrayShapeUtil` implements `canResize` and `onResize` with minimum bounds (360x180); `ArrayComponent` features elastic layout with `flex-1` centered slot area, `min-w-max` sync, and pinned footer. App also features a draggable split-pane divider to resize whiteboard/code workspace interactively. 54/54 unit tests passing across 8 test suites. `npm run lint` (`oxlint`) passes with 0 warnings/0 errors. `npm run build` (`tsc -b && vite build`) passes cleanly. Visual verification in Windows Chrome: QuickSort partition step 0 (ghost `[-1]` cell with $i=-1$, $j=0$, $pivot=4$), step 2 (swap $[29, 10] \to [10, 29]$ with fuchsia highlights), and step 5 (pivot 13 permanently sorted at index 1).
...
- [x] Create `VariableCardsShapeUtil` (`dsa-variable-cards`): first-class on-canvas TLDraw shape matching array and loop tracker panels.
- [x] Implement canvas pan/zoom tracking, bounding-box resizing (`canResize = () => true`), and position/size persistence across steps.
- [x] Direct manipulation: double-click card to edit value inline, + Add Var button to dynamically append variables.
...
- Verified: Created `src/ai/dsaIntentParser.ts` (Structured Intermediate Representation for arrays, linked lists, doubly linked lists, circular linked lists, trees, AVL trees, min/max heaps, stacks, queues, circular queues, graphs, hash tables, DP tables, recursion call stacks, variables, and algorithms with custom input arrays), `src/ai/dynamicAlgorithmStoryboards.ts` (tailored storyboards for second maximum, min/second-min, two-sum, linked list middle, bst search/find-min/insert, reverse array, binary search, bubble sort, stack operations with push/pop, circular queue ring buffer with modulo arithmetic, doubly linked list in-place reversal, circular linked list, AVL self-balancing rotations, min heap dual tree/array view, Dijkstra shortest path with priority queue, BFS traversal with queue reuse, DFS traversal with stack reuse, sliding window max subarray sum, 0/1 knapsack 2D DP table, and factorial recursion call stack), `src/ai/aiProblemSolver.ts`, `src/ai/featherlessClient.ts`, `src/ai/systemPrompt.ts` (configured with `zai-org/GLM-5.3-Flash` as primary recommended model), `src/ai/storyboardParser.ts`, `src/ai/microCommands.ts`, `src/ai/fallbackStoryboards.ts`, `src/canvas/shapes/StackShapeUtil.ts`, `src/canvas/shapes/QueueShapeUtil.ts`, and `src/components/chat/ChatPanel.tsx`. Fixed WhiteboardCanvas to dynamically render arrays, linked lists, trees, loops, stacks, and queues for all scenarios without hardcoded scenarioId restrictions. Added 6 new test suites (`tests/dsaStackQueue.test.ts`, `tests/dsaLinkedListExtensions.test.ts`, `tests/dsaTreeExtensions.test.ts`, `tests/dsaGraphAlgorithms.test.ts`, `tests/dsaAlgorithmicPatterns.test.ts`, and `tests/dsaComprehensiveCoverage.test.ts`). 180/180 unit tests passing across 24 suites in 589ms (`rtk npm test`). `rtk npm run lint` (`oxlint`) passes with 0 warnings/0 errors. `rtk npm run build` (`tsc -b && vite build`) passes in 1.14s. Captured Windows Chrome headless screenshots (`dsa_stack_visual.png`, `dsa_circular_queue_visual.png`, `dsa_dll_visual.png`, `dsa_heap_visual.png`, `dsa_studio_all_structures.png`). Browser cleanup verified (`rtk npm run cleanup:browsers`).
