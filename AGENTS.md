...
- **Scaffold (P0 & P1 done):** `dsa-studio` — npm + Vite 8 + React 19 + TypeScript (`tsc -b && vite build`). Scripts: `dev` / `build` / `lint` (oxlint) / `test` / `preview`. Deps: `@tldraw/tldraw` 5.4.0, tailwindcss 4.3.3 (v4 via `@tailwindcss/vite` plugin in `vite.config.ts`), lucide-react, clsx, tailwind-merge, canvas-confetti. P1 Shell & Layout complete: 35/65 split studio (`App.tsx`, `Header.tsx`, `CodePanel.tsx` with active line glow, `PromptBar.tsx`, `WhiteboardCanvas.tsx` with TLDraw and on-canvas Variable Cards, `TimelinePlayer.tsx` HUD with narration subtitle, and `SettingsModal.tsx`). Next: P2 Timeline Core & pure reducer.
