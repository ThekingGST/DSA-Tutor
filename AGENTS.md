...
- **Scaffold (P0, P1 & P2 done):** `dsa-studio` — npm + Vite 8 + React 19 + TypeScript (`tsc -b && vite build`). Scripts: `dev` / `build` / `lint` (oxlint) / `test` / `preview`. Deps: `@tldraw/tldraw` 5.4.0, tailwindcss 4.3.3 (v4 via `@tailwindcss/vite` plugin in `vite.config.ts`), lucide-react, clsx, tailwind-merge, canvas-confetti. P1 Shell & Layout and P2 Timeline Engine & Pure Reducer complete: pure deterministic state folding (`timelineReducer.ts`), `useTimeline` state machine hook, on-canvas live variable cards, and 19/19 unit tests passing. Next: P3 Array & Pointer TLDraw Custom Shape.
