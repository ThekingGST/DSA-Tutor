...
- **Scaffold (P0, P1, P2 & P3 done):** `dsa-studio` — npm + Vite 8 + React 19 + TypeScript (`tsc -b && vite build`). Scripts: `dev` / `build` / `lint` (oxlint) / `test` / `preview`. Deps: `@tldraw/tldraw` 5.4.0, tailwindcss 4.3.3 (v4 via `@tailwindcss/vite` plugin in `vite.config.ts`), lucide-react, clsx, tailwind-merge, canvas-confetti. P0-P3 complete: pure deterministic state folding (`timelineReducer.ts`), `useTimeline` state machine hook, on-canvas live variable cards, and custom TLDraw Array & Pointer shape (`ArrayShapeUtil`, `ArrayComponent`, `arrayShapeLogic`) with direct mouse manipulation, slot inline editing, pointer badge dragging/snapping, and 28/28 unit tests passing. Next: P4 Linked List & BST TLDraw Custom Shapes.
