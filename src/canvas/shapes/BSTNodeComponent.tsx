...
import type { ITreeNodeShape, BSTNodeShapeUtil } from './BSTNodeShapeUtil';
...
export const BSTNodeComponent: React.FC<BSTNodeComponentProps> = ({ shape, util }) => {
  ...
  return (
    ...
    >
      {/* Branch Label Badge (e.g. L or R) */}
      {branchLabel && (
        <span className="absolute -top-2 -left-2 z-10 w-5 h-5 rounded-full bg-amber-500 text-white font-mono text-[10px] font-bold flex items-center justify-center shadow-xs">
          {branchLabel}
        </span>
      )}
      ...
      <div
        ...
        className={`w-16 h-16 rounded-full border-2 flex flex-col items-center justify-center cursor-pointer transition-all duration-300 shadow-md ${
          isActive
            ? 'border-indigo-600 bg-indigo-50/95 ring-4 ring-indigo-400/80 scale-110 shadow-indigo-500/30'
            : isVisited
      ...
      >
        {isEditing ? (
          <input
            ...
            className="w-10 text-center font-handwriting text-xl font-bold text-indigo-700 bg-transparent border-b-2 border-indigo-600 focus:outline-hidden"
          />
        ) : (
